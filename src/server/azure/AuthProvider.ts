...
import { AzureTokenResponse, AzureUserInfoResponse, AzureMemberGroupResponse, AzureDirectoryObjectResponse } from "./AzureApiTypes";
...
const MEMBER_GROUPS_ENDPOINT = 'https://graph.microsoft.com/v1.0/me/getMemberGroups';
const GROUPS_INFO_ENDPOINT = 'https://graph.microsoft.com/v1.0/directoryObjects/getByIds';
...
export class AzureAuthProvider implements AuthProvider {
  ...
  async getToken(code: string, callbackUrl?: string): Promise<string> {
    ...
    const response = await got(this.tokenUrl, options).json() as AzureTokenResponse;
  ...
  async getUsername(token: string): Promise<string> {
    ...
    const response = await got(USER_INFO_ENDPOINT, options).json() as AzureUserInfoResponse;
  ...
  private async getGroupIds(token: string): Promise<string[]> {
    ...
    const response = await got(MEMBER_GROUPS_ENDPOINT, options).json() as AzureMemberGroupResponse;
    return response.value;
  }

  private async resolveGroupIds(token: string, groupIds: string[]): Promise<string[]> {
    const options = {
      method: "POST",
      headers: {
        Authorization: "Bearer " + token
    ...
    const response = await got(GROUPS_INFO_ENDPOINT, options).json() as AzureDirectoryObjectResponse;
